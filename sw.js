...
/* v101（2026-09-18）：这次改的是 app.js（加了云同步上传 + 服务端查书）。
   ⚠️ 离线能力不受影响，逐条对照过：
     - app.js / style.css 仍在 PRECACHE_URLS 里 → 断网照样能打开、能改数据
     - 图片仍在独立的 IMG_CACHE（cacheFirst，绝不回源）→ 离线封面不失效
     - 新增的 /api/* 调用全部指向 life-desk-api.pages.dev（跨域），
       而下面 fetch 处理器第 89 行「只处理同源」会直接放行 → SW 不掺和，
       断网时这些请求自然失败，代码里是静默降级（查书退回本地缓存/无结果，
       上传提示网络错误），不会卡住界面。
   所以 bump 版本号只是让新 app.js 尽快落地，不会牺牲离线。 */
const CACHE = 'lifedesk-v101-2026-09-18';
